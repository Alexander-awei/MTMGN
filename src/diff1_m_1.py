...
from my_src.vit import ViT
from src.text import tokenize, bert_embed, BERT_MODEL_DIM
from src.ped1_m_1 import pedMondel

class DDPM(nn.Module):
    def __init__(self, args, denoiser, cfg, device):
        super().__init__()

        self.model = pedMondel(cfg, args.frames, args.vel, args.seg)
    ...
    def cond_model(self, frame, seg, v, device):
        ...
        cond = self.model(frame, seg, v)

        return cond


    def calc_loss(self, prefix, gt, cond):
        ...
        denoise_result = self.denoiser(diff_sample, prefix, t, cond)
    ...
    def sample(self, prefix, pred_len, pose_dim, cond, use_zero=False, add_denoise_process=False):
        ...
        for t in range(self.cfg.diff_steps-1, -1, -1):
            ...
            diff_result = self.denoiser(curr_sample, prefix, time_vec, cond)
